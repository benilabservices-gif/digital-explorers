...
import { ArrowLeft, Shield, CheckCircle, XCircle, Star, Trophy, ArrowRight, Sparkles } from 'lucide-react';
import { WORLDS } from '@/data/content';
import type { Adventure, Lesson } from '@/data/content';
...
export default function AdventureSlugPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const slug = useEffect(() => { params.then(p => p.slug); }, [params]);
  ...
  useEffect(() => {
    ...
    // Find adventure in data
    for (const w of WORLDS) {
      const adv = w.adventures?.find((a: any) => a.slug === slug);
      if (adv) {
        setAdventure(adv);
        setWorld(w);
        // Generate sections from adventure data
        const secs: Section[] = [
          { id: 's1', section_type: 'story', order_num: 1, title: 'L\'histoire', content: adv.story },
          { id: 's2', section_type: 'discover', order_num: 2, title: 'Découvre', content: adv.description },
          { id: 's3', section_type: 'quiz', order_num: 3, title: 'Quiz', content: 'Teste tes connaissances !', quiz_questions: [
            { question: 'Quel est le point clé de cette aventure ?', options: ['A', 'B', 'C', 'D'], correct: 0 },
            { question: 'Pourquoi est-ce important ?', options: ['Oui', 'Non', 'Peut-être', 'Toujours'], correct: 0 },
          ]},
          { id: 's4', section_type: 'mission', order_num: 4, title: 'Mission', content: 'Applique ce que tu as appris dans un défi pratique.' },
          { id: 's5', section_type: 'project', order_num: 5, title: 'Projet', content: 'Crée quelque chose avec tes nouvelles compétences.' },
          { id: 's6', section_type: 'reward', order_num: 6, title: 'Récompense', content: `+${adv.xp_reward} XP débloqués !` },
        ];
        setSections(secs);
        break;
      }
  ...
  function handleQuizSubmit() {
    ...
    const correct = sec.quiz_questions?.[quizAnswer]?.correct === quizAnswer;
    ...
    <div className="min-h-screen bg-[#060810] text-white pb-32">
      ...
      <div className="pt-24 px-6 max-w-3xl mx-auto">
        ...
        <Link href={`/worlds/${world.slug}`} className="inline-flex items-center gap-2 text-gray-400 hover:text-white text-sm mb-6 transition-colors">
          <ArrowLeft className="w-4 h-4" /> Retour à {world.name}
        </Link>
        ...
        <div className="bg-[#111827] border border-white/5 rounded-xl p-6 mb-6 min-h-[200px]">
          {isQuiz ? (
            <div>
              ...
              {sec.quiz_questions?.map((q: any, qi: number) => (
                <div key={qi} className="mb-6">
                  <p className="font-semibold mb-3">{qi + 1}. {q.question}</p>
                  <div className="space-y-2">
                    {q.options?.map((opt: string, oi: number) => (
                      <button key={oi} onClick={() => !quizDone && setQuizAnswer(oi)}
                        className={`w-full text-left px-4 py-3 rounded-xl border transition-all ${quizAnswer === oi ? 'border-violet-500 bg-violet-500/20' : quizDone && oi === q.correct ? 'border-emerald-500 bg-emerald-500/20' : quizDone && quizAnswer === oi && oi !== q.correct ? 'border-red-500 bg-red-500/20' : 'border-white/10 hover:border-white/30'}`}>
                        {opt}
              ...
              {!quizDone ? (
                ...
                <div className="flex items-center gap-2 text-emerald-400"><Trophy className="w-5 h-5" /><span>Quiz terminé ! Score: {quizScore}/{sec.quiz_questions?.length}</span></div>
              )}
            </div>
          ) : (
            <>
              <div className="flex items-center gap-2 mb-3">
                {sec.section_type === 'story' && '📖'}
                {sec.section_type === 'discover' && '🔍'}
                {sec.section_type === 'mission' && '🎯'}
                {sec.section_type === 'project' && '🛠️'}
                {sec.section_type === 'reward' && '🏆'}
                <span className="text-sm text-violet-400 font-medium uppercase">{sec.section_type}</span>
              </div>
