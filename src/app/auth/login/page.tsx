...
export default function LoginPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    supabase.auth.signInWithPassword({ email, password }).then(({ data, error: authError }) => {
      ...
      if (data.user) {
        const profile = { id: data.user.id, pseudonym: data.user.email?.split('@')[0], xp: 0, level: 1 };
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-[#111827] border border-white/5 rounded-2xl p-8">
          ...
          <div className="mt-6 text-center text-sm text-gray-400">
            Pas encore de compte ? <Link href="/auth/signup" className="text-violet-400 hover:underline">Créer mon profil</Link>
          </div>
